...
const exponentialBackoffWithJitter = <T>({
  base,
  cap,
  timeout: timeoutMs,
  maxAttempts,
...
export type SendingWebSocketMessages =
  ...
  | {
      type: "start";
      id: string;
      payload: { data: string; extensions: { authorization: AuthHeaders } };
    };
...
type getAuthorizationHeaders = (args: {
  connect: boolean;
  data: { query?: string; variables?: { [name: string]: string } };
}) => AuthHeaders | Promise<AuthHeaders>;

export const appsyncRealtime = ({
  APIURL,
  ...
  closeDelay?: number;
  WebSocketCtor: WebSocketSubjectConfig<unknown>["WebSocketCtor"];
}) => {
  ...
  const realtimeUrl = APIURL.match(standardDomainPattern)
    ? APIURL.replace("appsync-api", "appsync-realtime-api").replace(
        "gogi-beta",
        "grt-beta"
      )
    : APIURL + "/realtime";

  const getAuthorizationHeadersSubject =
    new ReplaySubject<getAuthorizationHeaders>(1);
  ...
  const websockets = NEVER.pipe(
    ...
    (observable) =>
      new Observable<WebSocketSubject<WebSocketMessages>>((subscriber) => {
        const subscription = observable.subscribe({
          next: (authHeaders) => {
            const ws = webSocket<WebSocketMessages>({
              url: `wss://${new URL(realtimeUrl).host}${
                new URL(realtimeUrl).pathname
              }?header=${Buffer.from(
                JSON.stringify(authHeaders),
                "utf8"
              ).toString("base64")}&payload=${Buffer.from(
                JSON.stringify({}),
                "utf8"
              ).toString("base64")}`,
              protocol: "graphql-ws",
            ...
            const wsMessages = ws.pipe(
              tap(
                (message) =>
                  message.type === "error" &&
                  !("id" in message) &&
                  subscriber.error(message.payload)
              )
            );
            subscription.add(
              wsMessages
  ...
  return ({
      ...
      opened?: () => void;
      subscriptionRetryConfig?: RetryConfig;
    }) =>
    (query: string, variables: { [name: string]: string }) =>
      new Observable<object>((observer) => {
        getAuthorizationHeadersSubject.next(getAuthorizationHeaders);
        const subscriptionId = randomUUID();
        const subscription = websockets
          .pipe(
            exponentialBackoffWithJitter({
              base: connectionRetryConfig?.base ?? 10,
              cap: connectionRetryConfig?.cap ?? 2000,
              maxAttempts: connectionRetryConfig?.maxAttempts ?? 5,
              timeout: connectionRetryConfig?.timeout ?? 5000,
            }),
            (observable) =>
              new Observable<
                [
                  ws: WebSocketSubject<WebSocketMessages>,
              ...
              subscription.add(
                wsSubscription
                  .pipe(
                    ...
                    exponentialBackoffWithJitter({
                      base: subscriptionRetryConfig?.base ?? 10,
                      cap: subscriptionRetryConfig?.cap ?? 2000,
                      maxAttempts: subscriptionRetryConfig?.maxAttempts ?? 5,
                      timeout: subscriptionRetryConfig?.timeout ?? 5000,
                    }),
                    first(),
                    tap(opened),
                    mergeMap(() =>
...
export const persistentSubscription =
  (connection: ReturnType<typeof appsyncRealtime>) =>
  ({
    getAuthorizationHeaders,
    subscriptionRetryConfig,
    opened,
    closed,
    reopenTimeoutOnError,
    reopenTimeoutOnComplete,
  }: {
    getAuthorizationHeaders: getAuthorizationHeaders;
    subscriptionRetryConfig: RetryConfig;
    opened: () => unknown;
    closed: (error?: any) => unknown;
    reopenTimeoutOnError: number;
    reopenTimeoutOnComplete: number;
  }) =>
  (query: string, variables: { [name: string]: string }) =>
